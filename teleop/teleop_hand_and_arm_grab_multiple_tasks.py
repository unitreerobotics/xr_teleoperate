...
LEFT_HAND_SLICE = slice(0, 7)
RIGHT_HAND_SLICE = slice(7, 14)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teleop-side', type=str, choices=['left', 'right', 'both'], default='both',
    ...
    try:
        ...
        while not STOP:
            ...
            if args.ee == "fake_dex":
                ...
                left7 = fake_q14[:7]
                if args.teleop_side != "both" and 'dual_hand_state_array' in locals():
                    left7 = np.array(dual_hand_state_array[:7], dtype=np.float64) if not side_enabled(args.teleop_side, "left") else left7
                for i, jid in enumerate(Dex3_1_Left_JointIndex):
